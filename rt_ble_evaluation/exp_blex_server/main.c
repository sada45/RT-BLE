...
int main(void)
{
	...
	while (1){
        ...
        last_app = xtimer_now_usec();
		// rtt_printf("\n");
		// rtt_print_uint32((uint32_t)counter);
		// rtt_printf("-app=");
		// rtt_print_uint32(os_cputime_ticks_to_usecs(os_cputime_get32()));
        printf("\n%d-app=%lu", counter, os_cputime_ticks_to_usecs(os_cputime_get32()));
        ble_phy_output_duty_cycle();
		memset(_rxbuf, counter, P_LEN);
		ble_send(_rxbuf, P_LEN);
		counter++;
