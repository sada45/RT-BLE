...
int main(void)
{
    ...
    connsm = ble_ll_conn_find_by_handle(1);
	while (1){
		if (is_pause) {
			puts("paused");
        ...
        printf("\n%d-app=%lu,len=%d,ci=%lu", counter, os_cputime_ticks_to_usecs(os_cputime_get32()), transmit_len, connsm->conn_itvl * os_cputime_ticks_to_usecs(41));
