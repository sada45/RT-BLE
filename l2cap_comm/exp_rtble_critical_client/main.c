...
static uint8_t _txbuf[C_LEN];
#endif

uint8_t _rxbuf[2048] = { 0 };
...
static void _on_data(struct ble_l2cap_event *event)
{
    ...
    expect(rxd != NULL);
	if (event->receive.conn_handle == 1) {
		int rx_len = (int)OS_MBUF_PKTLEN(rxd);
		expect(rx_len <= (int)APP_MTU);
		os_mbuf_copydata(rxd, 0, rx_len, _rxbuf);
		if (_rxbuf[0] == 0x12 && _rxbuf[1] == 0x34) {
			thread_flags_set(_main, FLAG_ENTER_CRITICAL);
		}
		else if (_rxbuf[0] == 0x56 && _rxbuf[1] == 0x78) {
			thread_flags_set(_main, FLAG_LEAVE_CRTTICAL);
		}
	}
	/* free buffer */
...
int main(void)
{
	...
	while (1) {
		printf("start waiting flags at %lu\n", os_cputime_ticks_to_usecs(os_cputime_get32()));
		cmd_flag = thread_flags_wait_any(FLAG_ENTER_CRITICAL | FLAG_LEAVE_CRTTICAL);
		if (cmd_flag == FLAG_ENTER_CRITICAL) {
			printf("*****************\nEnter critical at %lu\n", os_cputime_ticks_to_usecs(os_cputime_get32()));
			res = rt_ble_enter_critical(1, 0, 1024, 100, 90);
			printf("start waitting, at %lu\n", os_cputime_ticks_to_usecs(os_cputime_get32()));
			sema_wait(&subrate_sema);
			printf("wait done at %lu\n", os_cputime_ticks_to_usecs(os_cputime_get32()));
			rt_ble_update_callback(1);
		}
		else if (cmd_flag == FLAG_LEAVE_CRTTICAL) {
			printf("*****************\nLeave critical at %lu\n", os_cputime_ticks_to_usecs(os_cputime_get32()));
			rt_ble_leave_critical(1);
			sema_wait(&subrate_sema);
			rt_ble_update_callback(1);
		}
	}
